...
-- 1. Verify view security_invoker setting
-- SELECT relname, reloptions FROM pg_class WHERE relname = 'admin_upcoming_deletions';
-- Expected: reloptions = {security_invoker=true}

-- 2. Verify function search_path settings
-- SELECT proname, proconfig FROM pg_proc
-- WHERE proname IN ('get_inactive_accounts', 'warn_inactive_accounts', 'cancel_deletion_on_activity');
