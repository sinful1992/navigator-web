    ...
    try {
      ...
      if (data.length === 0) {
        try {
          ...
          // First list the user's day folders so we don't rely on local date math
          const userPrefix = `${userId}`;
          const { data: dayEntries, error: dayListError } = await supabase.storage
            .from(bucket)
            .list(userPrefix, {
              limit: 50,
          ...
          if (dayListError) {
            const statusCode = (dayListError as any)?.statusCode ?? (dayListError as any)?.status;
            const message = String((dayListError as any)?.message || "").toLowerCase();
            if (statusCode !== 404 && !message.includes("not found")) {
              throw dayListError;
            }
          }

          const dayFolders = (dayEntries ?? [])
            .map((entry) => entry?.name)
            .filter((name): name is string => Boolean(name))
            .slice(0, 14); // check up to the 14 most recent folders in storage

          for (const dayKey of dayFolders) {
